...
export default function ProdutoVida() {
  const { t } = useTranslation('product_life');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white flex flex-col items-center py-12 px-4">
      <Seo
        ...
        canonicalPath={`/${base}/produto-vida`}
      />
  ...
  <div className="relative h-56 md:h-80 w-full flex items-center justify-center bg-blue-900">
          <img src="https://images.pexels.com/photos/1683975/pexels-photo-1683975.jpeg?auto=compress&w=800&q=60" alt={t('headerTitle')} className="absolute inset-0 w-full h-full object-cover opacity-30" />
          <div className="relative z-10 text-center w-full">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <Link to={`/${base}/simulacao-vida`} className="inline-block px-8 py-3 bg-yellow-400 text-blue-900 font-bold rounded-full shadow-lg hover:bg-yellow-300 transition">{t('ctaSimulate')}</Link>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <div key={idx} className="bg-blue-50 rounded-xl p-5 shadow flex flex-col items-start">
                  <span className="text-blue-700 text-2xl mb-2">{icon}</span>
                  <h3 className="font-bold text-blue-700 mb-1">{t(`types.${idx}.title`)}</h3>
                  <p>{t(`types.${idx}.desc`)}</p>
                </div>
              ))}
            </div>
          </section>
